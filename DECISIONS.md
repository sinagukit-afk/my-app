...
**Numbering scheme unified to SXX-prefix style + Chart of Accounts range discipline +
new Journal voucher numbers (2026-07-16).** Explicit request from Sinag to align
Purchase Order and Opex Expense reference formats with the `SOD`/`SQT`/`SPR`/`SSH`
convention, add a numbering-range discipline check to Chart of Accounts, and introduce a
Journal Entry voucher number (journal_entries previously had none at all).

- `purchase_orders.reference`: format changed from `SPO-YYYY-MMDDNNNN` to
  `SPOYY-MMDD-NNNN` (e.g. `SPO26-0716-0001`), matching every other prefix's style.
  `set_purchase_order_reference()` updated via `CREATE OR REPLACE`. Existing rows keep
  their old-format reference — not renumbered, same precedent as the SSP→SSH
  shipment-numbering change (see BUSINESS_RULES.md).
- `opex_expenses.expense_number`: prefix changed from `EXP-` to `SOE`, same format
  change (`EXP-YYYY-MMDDNNNN` → `SOEYY-MMDD-NNNN`, e.g. `SOE26-0716-0001`). Existing
  rows keep their old `EXP-` values, not renumbered.
- New `journal_entries.journal_number` column, `SJRYY-MMDD-NNNN` (e.g.
  `SJR26-0716-0027`), generated by a new `set_journal_number()` `BEFORE INSERT` trigger.
  The 26 pre-existing rows were backfilled in `created_at` chronological order (per-day
  sequence, matching what the trigger would have produced), then the column was set
  `NOT NULL UNIQUE`. Displayed in the Journal list (new "Journal No." column) and the
  Journal Entry detail page (title + summary card).
- **Correction to a prior assumption**: all of these prefix generators (`SOD`/`SQT`/
  `SPR`/`SSH`/`SPO`/`SRI`/now `SOE`/`SJR`) are actually **year-reset, not daily-reset** —
  the `LIKE` filter used to compute the next sequence number only matches on
  `PREFIX+YY`, not `PREFIX+YY+MMDD`, so `MMDD` in the string is a timestamp label only
  and does not bound the counting query. An earlier session's research (saved to Claude
  memory) had incorrectly described these as daily-reset; corrected here after
  inspecting the live trigger definitions directly.
- Chart of Accounts: added `check_account_number_range()`, a
  `BEFORE INSERT OR UPDATE OF account_number, category` trigger enforcing the numbering
  convention that was previously discipline-only (Asset 1000–1999, Liability 2000–2999,
  Equity 3000–3999, Revenue 4000–4999, Expense 5000–6999). Implemented as a trigger
  raising a plain-text exception rather than a bare `CHECK` constraint, so violations
  surface as a clean message (e.g. "Account number 9999 is out of range for asset
  accounts (must be 1000 - 1999).") through the existing `friendlyError()` fallback in
  `chart-of-accounts/actions.ts`, and to avoid a cast-order/short-circuit pitfall a bare
  `CHECK` would hit on non-numeric input. Confirmed no existing account (108 rows)
  violates its own category's range before applying.
- **Pitfall hit and fixed in the same session**: `CREATE OR REPLACE FUNCTION` on
  `set_purchase_order_reference()` and `set_opex_expense_number()` silently dropped
  their pre-existing `search_path=public` hardening (present on every sibling numbering
  trigger) because the replacement DDL didn't restate the `SET` clause. Caught by
  comparing `pg_proc.proconfig` across all numbering-trigger functions before/after;
  fixed with a follow-up `ALTER FUNCTION ... SET search_path = public` on both, and
  applied the same hardening to the two brand-new functions (`set_journal_number`,
  `check_account_number_range`) for consistency. **Any future
  `CREATE OR REPLACE FUNCTION` on an existing `SECURITY DEFINER` trigger function must
  explicitly restate `SET search_path = public` (or check `pg_proc.proconfig`
  afterward) — it is not preserved automatically.**
- All new/replaced functions are `SECURITY DEFINER` with
  `REVOKE ALL ... FROM PUBLIC, anon, authenticated`, matching the existing
  numbering-trigger convention.
- Verified via transaction-wrapped (`BEGIN`/`ROLLBACK`) smoke-test inserts against the
  live dev DB: confirmed exact output `SPO26-0716-0001`, `SOE26-0716-0001`,
  `SJR26-0716-0027`; a valid boundary account number (`1999`, asset) succeeds and an
  out-of-range one (`9999`, asset) is rejected with the expected message — no test rows
  persisted. `tsc --noEmit` clean. **Browser UI verification not completed this
  session**: another chat's dev server already owns port 3000 and this session's own
  preview launcher couldn't bind a second instance (its health-check appears hardcoded
  to port 3000 despite `autoPort`), so the new "Journal No." column and detail-page
  display were not visually confirmed live — only DB-level generation and the
  TypeScript-clean UI edits (mirroring the existing `entry_date`/`description`
  field-add pattern exactly) were verified.

## D050

**Entity code numbers (Customer/Fixed Asset/Supplier/Store/Courier/Receipt) +
Item SKU rebuilt as a locked-category dropdown (2026-07-16).** Explicit request
from Sinag, same session as D049.

- New master-data codes, all `PREFIX-NNNN` with no date component (lifetime
  running count, not year-scoped) — pattern per entity: `customers.customer_code`
  → `SCT-NNNNN` (5-digit; column already existed but was dead/unused, now
  activated), `fixed_assets.asset_code` → `SFX-NNNN`, `suppliers.supplier_code`
  → `SSP-NNNN`, `stores.store_code` → `SST-NNNN`, `couriers.courier_code` →
  `SCR-NNNN`. Each got a `BEFORE INSERT` trigger (`set_<entity>_code()`,
  `SECURITY DEFINER`, `search_path=public`, `REVOKE ALL FROM PUBLIC, anon,
  authenticated`), backfilled all existing rows chronologically by
  `created_at`, then `NOT NULL UNIQUE`. All five shown as a new "Code" column
  on their respective list screens (Customers also on its detail page header).
- `receipts.internal_reference` → `SRCYY-MMDD-NNNN`, **background only, no UI**
  per explicit instruction. Deliberately keyed off `receipt_date` (not
  `now()`/sync time) since Loyverse sync can lag the real sale by hours or
  days — using sync time would make the reference's date portion meaningless.
  This is the one prefix in the whole numbering family NOT keyed by insert
  wall-clock time; noted so a future session doesn't "fix" it back to `now()`
  for consistency without realizing why it's different.
- **Item SKU scheme rebuilt.** Live data showed the SKU convention already had
  an informal 4-prefix pattern tied to item naming (`SUIV-` "Itm-" raw
  materials, `SUPR-` "Pro-" finished products, `SUSV-` "Srv-/Shp-" services,
  `SUSP-` "Pkg-" packaging) — never enforced, just manually followed. New
  `item_variants.sku_category` column (`SIM`/`SSV`/`SPK`/`SPR`/`STS`, CHECK
  constrained, NOT NULL) plus a CHECK that `sku` actually starts with
  `sku_category || '-'`. Existing SKUs renamed by prefix-swap only (numeric
  suffix preserved exactly, no renumbering): `SUIV`→`SIM` (32), `SUPR`→`SPR`
  (19), `SUSV`→`SSV` (4), `SUSP`→`SPK` (4); the 3 unprefixed test-artifact
  rows (`ITEM65-COMPOSITE-001` etc., leftover from the ITEM-6.5 session) became
  `STS-0001..0003` — `STS` has no historical equivalent, it's a genuinely new
  5th category for future/test items per Sinag's answer when asked.
  New `next_item_sku(p_category)` function (`SECURITY DEFINER`, role-gated
  admin/manager like `upsert_item`, granted `EXECUTE` to `authenticated` only
  — the one exception to this session's "revoke from authenticated too"
  pattern, since the client legitimately calls this one directly) computes the
  next `PREFIX-NNNN` by `max(existing suffix for that prefix) + 1` — **max,
  not count**, because real gaps already exist in the historical numbering
  (e.g. `SUIV`/`SIM` jumps `0029`→`0031`) and a count-based approach would
  have immediately collided with the existing `SIM-0033`.
  `item-form.tsx`'s free-text SKU input became a required "SKU Category"
  dropdown + a disabled/read-only SKU field that calls `next_item_sku` (via a
  new `previewItemSku` server action) whenever the category changes; the
  fetched value is what actually gets submitted. Edit mode seeds the dropdown
  from the variant's real `sku_category` and shows its real existing `sku` —
  it does **not** fetch a fresh preview on mount, only on an actual category
  change.
  `upsert_item` (the single RPC that creates/edits all item_variants rows —
  confirmed via `prosrc ilike` search, no other INSERT path exists) was
  edited in place: same signature (no duplicate-overload risk), added a
  `sku_category` validation check alongside the existing SKU check, and reads
  `sku_category` off each submitted variant into both its INSERT and UPDATE
  branches. The `_variant_map` temp table's correlation key is still `sku`
  (unchanged) — since the client now gets a real `sku` value back from
  `next_item_sku` *before* building the submission payload, the existing
  sku-keyed correlation logic (composite components reference their own
  variant's sku) still works without needing a deeper rewrite.
- **Two pitfalls caught and fixed in this same session, both recurrences of
  the D049 `search_path` lesson ([[feedback_create_or_replace_function_signature_change]]):**
  (1) the `upsert_item` `CREATE OR REPLACE` correctly restated
  `SET search_path TO 'public'` this time (learned from D049) and verified
  clean via `pg_proc.proconfig` immediately after applying — no regression.
  (2) `check_account_number_range()` and `set_journal_number()` — both created
  in the *previous* D049 migration — were written without `SECURITY DEFINER`
  at all (not a search_path drop, a flat omission), inconsistent with every
  sibling trigger function. Caught by a `pg_proc` sweep across all
  numbering/validation functions after this session's work, not during D049
  itself. For `set_journal_number()` this was a real correctness gap (it
  `SELECT`s from RLS-restricted `journal_entries`, so behavior under
  `SECURITY INVOKER` would depend on the calling session's own RLS
  visibility); for `check_account_number_range()` it was functionally inert
  (no cross-table reads) but fixed anyway for consistency. Fixed via
  `ALTER FUNCTION ... SECURITY DEFINER` on both, re-verified via `pg_proc`.
  **Any new trigger/validation function in this project should be checked
  against `pg_proc.prosecdef`/`proconfig` right after creation, not assumed
  correct from the DDL alone** — this is now twice in two sessions that a
  hand-written function silently diverged from the established hardening
  pattern.
- Verified live end-to-end in a real browser session (Claude admin test
  account, port 3000 — the other concurrent session's `next dev` process had
  already exited on its own by the time this was checked; nothing was force-
  killed): Journal No. column (carried over from D049, now actually seen
  live), Code columns on Customers/Fixed Assets/Suppliers/Stores/Couriers all
  render correctly. Item SKU flow: category dropdown correctly offline-
  generates `SIM-0034` then `STS-0004` on selection change, field confirmed
  `disabled`; a real test item ("D050 SKU scheme smoke test") was created
  through the actual form submission (not direct SQL), appeared correctly as
  `STS-0004` in both the list and detail pages, then archived (soft-deleted)
  through the UI's own Archive dialog to clean up — confirmed
  `items.deleted_at` set, no hard delete. Edit mode on a real existing item
  (`Itm-Keychain Leather, Rectangle`) confirmed the dropdown/field correctly
