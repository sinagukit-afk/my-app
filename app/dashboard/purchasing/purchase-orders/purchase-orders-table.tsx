  ...
  const columns: Column<PurchaseOrderRow>[] = [
    {
      ...
      sortable: true,
      render: (value, row) => (
        <Link href={`/dashboard/purchasing/purchase-orders/${row.reference}`} className="font-medium text-(--color-primary) hover:underline">
          {String(value)}
        </Link>
      ),
    },
    ...
    },
    {
      ...
      render: (_value, row) => (
        <div className="flex items-center gap-2">
          <Link href={`/dashboard/purchasing/purchase-orders/${row.reference}`}>
