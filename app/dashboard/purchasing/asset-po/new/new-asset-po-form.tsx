...
export function NewAssetPurchaseOrderForm({ suppliers, categories }: Props) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        router.push(`/dashboard/purchasing/asset-po/${res.reference}`);
      } else {
        setError(res.error);
      }
    });
  ...
  return (
    <form onSubmit={handleSubmit} onKeyDown={handleFormKeyDown} className="space-y-6">
      <PageHeader title="New Asset PO" description="Request approval to purchase a fixed asset before buying." />
