...
export async function createExpensePurchaseOrder(formData: FormData): Promise<CreateResult> {
  ...
  const { data: po, error } = await supabase
    ...
    .insert({ po_type: 'expense', supplier_id, order_date, expected_date, shipping_fee, note, created_by: user?.id ?? null })
    .select('id, reference')
    .single()
...
export async function updateExpensePurchaseOrderHeader(
  id: string,
  reference: string,
  formData: FormData
...
export async function setExpensePurchaseOrderStatus(
  id: string,
  reference: string,
  nextStatus: string
): Promise<ActionResult> {
...
export async function deleteExpensePurchaseOrder(id: string): Promise<ActionResult> {
  const supabase = await createClient()
...
export async function addExpensePurchaseOrderItem(
  purchaseOrderId: string,
  reference: string,
  formData: FormData
): Promise<ActionResult> {
  const expense_category_id = formData.get('expense_category_id') as string
...
export async function removeExpensePurchaseOrderItem(
  purchaseOrderId: string,
  reference: string,
  itemId: string
): Promise<ActionResult> {
...
export async function receiveExpensePurchaseOrder(
  purchaseOrderId: string,
  reference: string,
  lines: { po_item_id: string; quantity: number }[]
): Promise<ActionResult> {
  const supabase = await createClient()
  const { error } = await supabase.rpc('receive_expense_purchase_order', {
