...
import { SALES_MAPPING_KEYS, MAPPING_KEY_ACCOUNT_CATEGORY } from "@/lib/accounting/system-mapping-keys";
import { saveSalesMappings } from "./actions";
...
export default async function SalesMappingPage() {
  ...
  const rows: SystemMappingRow[] = SALES_MAPPING_KEYS.map((key) => {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-4">
          <SystemMappingTable rows={rows} accounts={(accounts ?? []) as AccountOption[]} canEdit={canEdit} onSave={saveSalesMappings} />
        </CardContent>
      </Card>
    </div>
  );
