...
function friendlyError(error: { code?: string; message: string }): string {
  if (error.code === '42501') return 'You do not have permission to edit Inventory Mapping.'
  if (error.code === '23503') return 'One of the selected accounts no longer exists.'
  return error.message
}

export async function saveInventoryMappings(rows: MappingInput[]): Promise<SaveResult> {
