...
export function QuoteDetail({ data, logs }: { data: QuoteDetailData; logs: ActivityLogRow[] }) {
  ...
  function handleConvert() {
    if (!confirm(`Convert ${data.quoteNumber} to a Sales Order? This will reserve available stock.`)) return;
    startTransition(async () => {
      const res = await convertQuote(data.id);
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {data.convertedOrderId && (
                  <Link
                    href={`/dashboard/orders/order-list/${data.convertedOrderId}/edit`}
