...
export default async function QuoteDetailPage({ params }: { params: Promise<{ quoteNumber: string }> }) {
  ...
  const { data: convertedOrder } = quote.converted_order_id
    ? await supabase.from("orders").select("order_number").eq("id", quote.converted_order_id).single()
    : { data: null };
  ...
  const data: QuoteDetailData = {
    ...
    convertedOrderNumber: convertedOrder?.order_number ?? null,
