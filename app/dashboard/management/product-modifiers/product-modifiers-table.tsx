...
export function ProductModifiersTable({ data, canWrite, canDelete }: Props) {
  ...
  async function handleArchive(row: ModifierRow) {
    if (!confirm(`Archive modifier "${row.name}"? It will stop showing up as an option on new items.`)) return;
    const res = await archiveModifier(row.id);
