...
import { createIncomingItem, type ActionResult } from "./actions";
...
export function IncomingForm({ open, onOpenChange, suppliers, items, onSaved }: Props) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res: ActionResult = await createIncomingItem(formData);
      if (res.success) {
        onSaved();
        onOpenChange(false);
      } else {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <DialogHeader>
            <DialogTitle>Log Incoming Inventory</DialogTitle>
            <DialogDescription>
              Record a manual inventory receipt. The stock level will be updated automatically.
            </DialogDescription>
          </DialogHeader>
