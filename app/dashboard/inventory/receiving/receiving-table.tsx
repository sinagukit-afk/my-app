  ...
  const columns: Column<ReceivablePO>[] = [
    {
      ...
      header: "Reference",
      render: (value, row) => (
        <Link href={`/dashboard/inventory/receiving/${row.reference}`} className="font-medium text-(--color-primary) hover:underline">
          {String(value)}
        </Link>
      ),
    },
    ...
    { key: "items_remaining", header: "Units Remaining" },
    {
      ...
      header: "",
      render: (_value, row) => (
        <Link href={`/dashboard/inventory/receiving/${row.reference}`} className="text-sm text-(--color-primary) hover:underline">
          Receive →
        </Link>
      ),
    },
