...
## PS-23 — AI Form Auto-Fill extended to Add Shipment ✅ DONE (2026-07-12)

**Requested by Sinag directly**, with a sample photo of a J&T Express shipping label attached. See [[project_ai_autofill_module]] for the shared module's architecture — this is its 5th form integration (after Expense PO, Asset PO, Inventory PO, Manual Incoming), and the first that's a `Dialog` rather than a full page.

**What was built:**
- Rewrote the previously-unused `shipping_label` schema (`lib/ai-autofill/schemas/shipping-label.ts`) from its original 5 generic fields to 13 fields matching the "Add Shipment" dialog in `order-shipments.tsx` exactly: `courierId` (dropdown), `trackingNumber` (required), the full receiver address breakdown (`receiverName`/`Phone`/`AddressLine1`/`Barangay`/`City`/`Province`/`PostalCode`), `shippingCost`, plus `weight`/`goodsDescription`/`courierOrderNo` which have no dedicated form field and instead get composed into the free-text Notes field — joined with `" | "`, not `"\n"`, since Notes is a single-line `Input` not a `TextArea` and a newline-joined string silently ran together unreadably (caught and fixed same session).
- Wired `AutoFillPanel`/`AiFieldHighlight`/`useAiFilledKeys` into `order-shipments.tsx` following the established PO-form pattern. Two UX judgment calls made without being asked: a courier/tracking match flips `fulfillmentType` to `"delivery"` (a courier label is obviously not a pickup); an extracted receiver name/address flips `shipsToCustomer` off — even when the order has a registered customer — so the label's actual, photographed receiver is shown for review rather than silently deferring to the customer record. `resetForm()` (called by `openAddForm()`) now also calls `useAiFilledKeys().clearAll()`, since this is a reused `Dialog` and not a full page — earlier consumers never needed this.
- **Bug reported by Sinag after trying his own real photo**: the AI was pulling the SENDER's name/address into the `receiver*` fields instead of the RECEIVER's. Root cause: `openai-vision.ts` only ever sent the JSON-schema property key (e.g. `"receiverName"`) to the model, with no `"description"` — a real courier label prints both a sender/return-address block and a receiver/consignee block in similar layout/font, so the key name alone wasn't a strong enough signal. Fixed generically, not shipping-label-specifically: added an optional `FieldSchema.description` (`lib/ai-autofill/types.ts`), wired into `openai-vision.ts`'s `fieldJsonSchema()` as that property's JSON-schema `description` (a standard keyword, fully compatible with OpenAI's `strict: true` structured outputs). Every `receiver*` field in `shipping-label.ts` now carries an explicit "this is the RECEIVER/consignee... the label also prints a separate SENDER/return-address block... never use that one here" instruction. `FieldSchema.description` is available to any other schema that hits similar key-name ambiguity later.

**Verified (browser preview + direct API calls, Claude admin test account):** `npx tsc --noEmit` clean. Couldn't access the user-pasted photo's raw bytes (no filesystem path was exposed for it), so built two synthetic replica images via `sharp` (already in `node_modules`) rendering an SVG, injected into the hidden file input via the `DataTransfer` API trick in page-context JS (`dt.items.add(file); input.files = dt.files; input.dispatchEvent(new Event('change', {bubbles:true}))`) — bypasses the native OS file picker, which the browser automation tools can't drive.
1. First pass (image with labeled sub-fields, hit the free local-OCR path): all 13 fields extracted and mapped correctly — courier "J&T Express" text fuzzy-matched to the registered "J&T" dropdown option, the 19-digit `courierOrderNo` survived intact (typed `"string"`, not `"number"` — would've lost precision as a JS float). Actually submitted the shipment (`SSH26-0712-0036` via RPC `create_shipment`) to confirm the full server-action path, then deleted it via direct SQL (status stayed `preparing`, so no stock movement to reverse) — order confirmed back to "No shipments yet."
2. Second pass, after the sender/receiver bug was reported: called `/api/ai-autofill/extract` directly to force the real AI vision path (bypassing the local-OCR hybrid entirely), against an image with both a "Receiver:" block (Ma Khatrina Andres/Isabela) and a "Sender:" block (Sylvia Valdez/Laguna) worded as freeform paragraphs like the real label. Response came back `"source":"ai"` with `receiverName: "Ma Khatrina Andres"` and her address — correctly ignoring the sender, confirming the fix.

**Depends on:** PS-6/PS-8 (the Shipments feature this dialog belongs to) and the AI Auto-Fill module ([[project_ai_autofill_module]]). Independent of everything else in this doc.
