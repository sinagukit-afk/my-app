...
|---|---|---|---|
| 1 | Sale recognized | `close_order_payment()` | **Corrected 2026-07-10 (ACCT-7.5)**: no single generic "Sales Revenue" account exists in the real Chart of Accounts — revenue splits per order line by its mapped `item_accounting_mappings.revenue_account_id`, same granularity as COGS. Paid/Overpaid: `Dr Cash/Bank [per payment-type account] / Cr Sales Revenue - <product line> [per-line share of order total]`; Overpaid adds `Dr Cash/Bank [tip] / Cr Tip Income [tip]`; Partially Paid: `Dr Cash/Bank [amount collected] + Dr Write-off Expense [uncollected] / Cr Sales Revenue - <product line> [full order total, split per line]`. Which Cash/Bank account depends on payment method, resolved via new `payment_type_accounting_mappings` (Cash → Cash on hand; BDO/BPI/Gcash/Maribank/Other → Bank Account, confirmed with Sinag) |
| 2 | Cost of goods sold | ~~`mark_shipment_shipped()` / `mark_shipment_picked_up()` (both call `deduct_stock_out()`)~~ **corrected 2026-07-10**: both call a shared private helper, `_deduct_shipment_stock()`, which loops components/packaging and calls `deduct_stock_out()` per line — the event is wired there (one `cogs` event per shipment, `lines` array payload) | `Dr Material Expense - <product line> / Cr Inventory - <material category>`, at item cost × qty. Reuses the existing per-product Material Expense accounts (5001–5021) rather than one generic COGS account. |
| 3 | Purchase received | ~~`receive_purchase_order()`~~ **corrected 2026-07-10**: `receive_purchase_order()` and Manual Incoming both insert into `incoming_items`, which fires `apply_incoming_item_inventory_movement()` — that shared `AFTER INSERT` trigger is the real hook, branching on `new.source` | Ordinary payment: `Dr Inventory - <material category> / Cr Cash/Bank`. Credit card: `Dr Inventory - <material category> / Cr Credit Card Payable` |
| 4 | Manual incoming | `apply_incoming_item_inventory_movement()` — same trigger as #3, `new.source = 'manual'` branch (confirmed 2026-07-10, not a separate hook) | Same as #3 (always a real purchase per decision 8) |
| 5 | Inventory adjustment (gain) | `adjust_stock()`, `p_qty_delta > 0` branch — confirmed 2026-07-10 | `Dr Inventory - <category> / Cr Inventory Adjustment Gain` |
| 6 | Inventory adjustment / scrap (loss) | Two sources, both wired 2026-07-10: `adjust_stock()` `p_qty_delta < 0` branch (generic count corrections), and new dedicated RPC `release_to_scrap()` (Items for Review → Release to Scrap — previously called `deduct_stock_out()` directly with no event hook, now wraps it) | `Dr Inventory Shrinkage/Scrap Expense / Cr Inventory - <category>` |
